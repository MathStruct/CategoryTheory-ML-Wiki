  ...
  for (const line of src.split("\n")) {
    if (fence) {
      if (line.trim() === fence.marker) {
        cur!.nodes.push({ type: "code", lang: fence.lang || null, value: fence.lines.join("\n") } as Code)
        fence = null
    ...
    return [
      () => (tree: Root) => {
        visit(tree, "code", (node: Code, index, parent) => {
          ...
          const nav = tabs
            .map((t, i) => `<button type="button" class="tab-button${i === 0 ? " active" : ""}" data-tab="${i}">${escape(t.title)}</button>`)
            .join("")
          const out: RootContent[] = [{ type: "html", value: `<div class="obsidian-tabs"><div class="tab-nav" role="tablist">${nav}</div>` } as any]
          tabs.forEach((t, i) => {
            out.push({ type: "html", value: `<div class="tab-panel${i === 0 ? " active" : ""}" data-tab="${i}">` } as any)
            out.push(...t.nodes)
