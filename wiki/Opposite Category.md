...
- A [[Functor]] $F : \mathcal{C} \to \mathcal{D}$ has an opposite $F^{\mathrm{op}} : \mathcal{C}^{\mathrm{op}} \to \mathcal{D}^{\mathrm{op}}$, the same on objects and $F^{\mathrm{op}}(f^{\mathrm{op}}) := F(f)^{\mathrm{op}}$ ([[7S Exercise 3.101]]).
