...
- **Power set** $\mathcal{P} : (\mathbf{Set}, 1, \times) \to (\mathbf{Set}, 1, \times)$ with $\mathcal{P}(f) = $ direct image, $\varphi_I(1) = \{1\}$, $\varphi_{S,T}(A, B) = A \times B$ (Example 6.69; naturality is [[7S Chapter 6 Exercises#Exercise 6.70|7S Exercise 6.70]]).
- **Decoration functors** $(F, \varphi) : (\mathbf{FinSet}, \varnothing, +) \to (\mathbf{Set}, 1, \times)$, e.g. $\mathbf{Circ}$ sending a finite set of nodes to the set of circuits on it, with $\psi_{V,V'}$ the disjoint union of circuits (Eq. 6.81); these build [[Decorated Cospan|decorated cospan]] categories (Theorem 6.77). The constant functor $F(c) = \{\ast\}$ recovers plain cospans ([[7S Chapter 6 Exercises#Exercise 6.78|7S Exercise 6.78]]).
- $U : \mathbf{Mat}(R) \to \mathbf{Set}$, $n \mapsto R^n$, strong monoidal since $R^m \times R^n \cong R^{m+n}$ ([[7S Chapter 5 Exercises#Exercise 5.69|7S Exercise 5.69]]); monoidal functors carry [[Monoid Object|monoid objects]] to monoid objects.
