...
- `show :: Int -> String` is **not** an algebra morphism from `eval` to `pretty`: on a `PlusF 2 3` node, `show (eval ..) = "5"` but `pretty (fmap show ..) = "2 + 3"` ([[DaoFP Exercise 12.2.1]]). By contrast `log` *is* a morphism from the multiplicative to the additive algebra of `FloatF` ([[DaoFP Exercise 12.2.2]]).
