...
- In a [[Preorder]], a terminal object is a **top element** $z$ with $c \leq z$ for all $c$ ([[7S Exercise 3.81]]) — the empty [[Meet]].
- In [[Category of Categories|$\mathbf{Cat}$]], the one-object category $\underline{\mathbf{1}}$ ([[7S Exercise 3.82]]); in $\mathbf{Grph}$, the graph with one vertex and one loop; in [[Category of Preorders|$\mathbf{Preord}$]] the one-point preorder.
