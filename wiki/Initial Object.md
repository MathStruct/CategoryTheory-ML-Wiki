...
- In $\mathbf{Set}$: $\varnothing$ (unique empty function); a map *into* $\varnothing$ forces the domain empty ([[7S Exercise 1.25]]) — in a [[Cartesian Closed Category]] the initial object is *strict*: "we'll assume it has no incoming arrows other than its identity, so `Void` has no elements" and is empty.
