...
- In $\mathbf{Cospan}_{\mathcal{C}}$ every object: $\mu_X = (X + X \xrightarrow{[\mathrm{id}, \mathrm{id}]} X \xleftarrow{\mathrm{id}} X)$, $\eta_X = (\varnothing \to X \xleftarrow{\mathrm{id}} X)$, $\delta_X = (X \xrightarrow{\mathrm{id}} X \xleftarrow{[\mathrm{id},\mathrm{id}]} X + X)$, $\varepsilon_X = (X \xrightarrow{\mathrm{id}} X \leftarrow \varnothing)$ (Example 6.61; the special law is the pushout square $X + X \rightrightarrows X$, [[7S Exercise 6.63]]; pictures in [[7S Exercise 6.62]]).
...
- Frobenius monoids give a self-dual [[Compact Closed Category|compact closed]] structure: cup $\eta \mathbin{;} \delta$, cap $\mu \mathbin{;} \varepsilon$ (Proposition 6.66, [[7S Exercise 6.67]]).
